...
class UserSerializer(serializers.ModelSerializer):
    duenio = serializers.PrimaryKeyRelatedField(
        many=True, queryset=Pelicula.objects.all()
    )
...
class DetallesSerializer(serializers.ModelSerializer):
    nombre_representacion_pelicula = serializers.CharField(
        source="nombre.nombre", read_only=True
    )
...
class PeliculaSerializer(serializers.ModelSerializer):
    ...
    representacion_detalles = DetallesSerializer(
        source="detalles", many=True, read_only=True
    )
    direccion = serializers.CharField(
        source="director_pelicula.director", read_only=True
    )
    banda_sonora = serializers.PrimaryKeyRelatedField(
        queryset=Musica.objects.all(), many=True, write_only=True
    )
    ...
    def create(self, validated_data):
        ...
        if len(musica_data) != 0:
            ...
            raise serializers.ValidationError(
                "ERROR: Tienes que seleccionar la banda sonora"
            )

    def update(self, instance, validated_data):
        instance.nombre = validated_data.get("nombre", instance.nombre)
        instance.genero_pelicula = validated_data.get(
            "genero_pelicula", instance.genero_pelicula
        )
        instance.director_pelicula = validated_data.get(
            "director_pelicula", instance.director_pelicula
        )
        instance.clasificacion = validated_data.get(
